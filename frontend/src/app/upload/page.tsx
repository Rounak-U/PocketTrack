...
const SidebarComponent = ({ activeItem, setActiveItem }: { activeItem: string, setActiveItem: (item: string) => void }) => {
  ...
  const LogoWrapper = () => {
    // useSidebar is safe inside this child because it will run after SidebarProvider mounts
    try {
      // dynamic require to avoid top-level hook issues when server rendering during patching

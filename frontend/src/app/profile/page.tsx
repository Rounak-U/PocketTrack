...
const Profile = () => {
  ...
  const handleConfirmDelete = async () => {
    if (!userData?.email) {
      setDeleteError('Unable to load your email for confirmation.');
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1">
        <div
          className={cn(
            "p-4 sm:p-6 md:p-8 rounded-tl-2xl border border-zinc-900 bg-gradient-to-br from-neutral-950 via-zinc-950 to-neutral-900 backdrop-blur-sm flex flex-col gap-4 sm:gap-6 flex-1 w-full md:h-full relative overflow-hidden",
            showDeleteConfirm ? "overflow-hidden" : "md:overflow-y-auto"
          )}
        >
          <div className="absolute inset-0 pointer-events-none opacity-70" aria-hidden>
            <div className="absolute -left-24 top-10 h-64 w-64 bg-emerald-500/10 blur-3xl" />
            <div className="absolute right-0 -top-10 h-64 w-64 bg-sky-500/10 blur-3xl" />
            <div className="absolute -bottom-16 left-10 h-64 w-72 bg-violet-500/10 blur-3xl" />
          </div>

          <div className="relative mb-4 sm:mb-6">
            <div className="overflow-hidden rounded-3xl border border-zinc-800 bg-gradient-to-br from-zinc-950/90 via-neutral-950 to-slate-950 shadow-2xl p-5 sm:p-7">
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
                <div className="relative">
                      <UserAvatar compact={true} size={96} />
                  <div className="absolute -bottom-1 -right-1 px-3 py-1 rounded-full bg-emerald-500/90 text-[11px] font-semibold text-white shadow-lg">
                    Active
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                    <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs uppercase tracking-wide text-emerald-200">
                      <User className="w-3.5 h-3.5" /> Profile
                    </span>
                    <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-xs text-emerald-100">
                      <CalendarIcon className="w-3.5 h-3.5" /> {`Member since ${formatJoinDate(userData.joinDate)}`}
                    </span>
                  </div>
                  <h1 className="mt-2 text-2xl sm:text-3xl font-semibold text-white leading-tight">{userData.name || 'Your profile'}</h1>
                  <p className="mt-1 text-sm text-zinc-400 flex flex-wrap items-center gap-2">
                    <span className="inline-flex items-center gap-1.5 text-zinc-300"><Mail className="w-3.5 h-3.5" /> {userData.email || 'Add email'}</span>
                    {userData.location ? <span className="inline-flex items-center gap-1.5 text-zinc-400"><MapPin className="w-3.5 h-3.5" /> {userData.location}</span> : null}
                  </p>
                  <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
                    {[
                      { label: 'Transactions', value: accountStats.totalTransactions, tone: 'from-emerald-500/15 to-emerald-500/5', icon: <Wallet className="w-4 h-4" /> },
                      { label: 'Categories', value: accountStats.activeCategories, tone: 'from-sky-500/15 to-sky-500/5', icon: <PieChart className="w-4 h-4" /> },
                      { label: 'Savings Rate', value: accountStats.savingsRate > 0 ? `${accountStats.savingsRate}%` : '—', tone: 'from-amber-500/15 to-amber-500/5', icon: <Target className="w-4 h-4" /> },
                      { label: 'Insights', value: accountStats.monthlyInsights, tone: 'from-violet-500/15 to-violet-500/5', icon: <Lightbulb className="w-4 h-4" /> }
                    ].map((chip, idx) => (
                      <div key={idx} className={cn("rounded-xl border border-zinc-800 px-3 py-2.5 bg-gradient-to-br", chip.tone)}>
                        <div className="flex items-center justify-between text-[11px] text-zinc-400">
                          <span className="flex items-center gap-1.5">{chip.icon}{chip.label}</span>
                          <ArrowUpRight className="w-3 h-3 text-zinc-500" />
                        </div>
                        <p className="mt-1 text-sm sm:text-base font-semibold text-white">{chip.value}</p>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="flex flex-row sm:flex-col gap-2 sm:gap-3">
                  {!isEditing ? (
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-emerald-500 text-white text-sm font-semibold shadow-lg shadow-emerald-500/20"
                    >
                    ...
                    <div className="flex flex-row sm:flex-col gap-2">
                      ...
                      <button
                        ...
                        className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-zinc-800 text-zinc-100 text-sm font-semibold border border-zinc-700"
                      >
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-white/5 text-zinc-100 text-sm font-semibold border border-zinc-800 hover:bg-white/10 transition-colors"
                  >
          ...
          {!loadingProfile && !profileError && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 transition-all">
            {/* Profile Information */}
              ...
              {showDeleteConfirm && (
                <div className="fixed inset-0 bg-black/75 z-50 flex items-center justify-center p-4">
                  <motion.div
                    ...
                    className="bg-zinc-900 border border-rose-800 rounded-xl p-4 sm:p-6 max-w-lg w-full shadow-2xl min-h-[320px]"
                  >
                    <div className="flex items-start gap-3 sm:gap-4 mb-3 sm:mb-4">
                      <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br from-rose-500/30 to-rose-600/20 border border-rose-500/40 flex items-center justify-center shadow-[0_0_0_1px_rgba(244,63,94,0.25)]">
                        <AlertTriangle className="w-5 h-5 sm:w-6 sm:h-6 text-rose-100" />
                      </div>
                      <div className="flex-1 min-w-0 space-y-1">
                        <h3 className="text-lg sm:text-xl font-semibold text-rose-50">Delete account</h3>
                        <p className="text-xs sm:text-sm text-rose-100/80">
                          This is permanent. All data will be removed and cannot be recovered.
                        </p>
                        <p className="text-[11px] text-rose-100/60">Type your account email to continue.</p>
                      </div>
                    </div>

                    <div className="mb-4 sm:mb-6">
                      <label htmlFor="delete-confirm" className="block text-xs sm:text-sm font-medium text-rose-50 mb-2">
                        Confirmation (enter your email)
                      </label>
                      <input
                        id="delete-confirm"
                        type="text"
                        ...
                        className="w-full px-3 sm:px-4 py-2 sm:py-2.5 bg-rose-900/40 border border-rose-800 rounded-lg text-sm text-rose-50 placeholder-rose-200/60 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-transparent"
                        disabled={isDeleting}
                        autoFocus
                      />
                      {deleteError && (
                        <p className="mt-2 text-xs sm:text-sm text-rose-200">{deleteError}</p>
                      )}
                    </div>
